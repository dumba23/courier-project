...
export function DeliveryItemsPage({ auth, viewScope = "active" }) {
  ...
  const storedTableState = useMemo(
    () => readStoredTableState(viewScope),
    [viewScope],
  );
  ...
  const [additionalStatusUpdateId, setAdditionalStatusUpdateId] =
    useState(null);
  ...
  const [filters, setFilters] = useState(
    storedTableState?.filters ?? initialFilters,
  );
  const [sort, setSort] = useState(storedTableState?.sort ?? initialSort);
  const [pagination, setPagination] = useState(
    storedTableState?.pagination ?? initialPagination,
  );
  const [visibleColumns, setVisibleColumns] = useState(
    storedTableState?.visibleColumns ?? initialVisibleColumns,
  );
  const [meta, setMeta] = useState(initialPagination);
  const [reloadKey, setReloadKey] = useState(0);
  const [draftItems, setDraftItems] = useState(() => [
    createEmptyDraftItem(auth?.user?.role === "admin"),
  ]);
    ...
    () =>
      Object.fromEntries(
        partners.map((partner) => [
          partner.user?.email?.toLowerCase(),
          partner,
        ]),
      ),
    [partners],
  ...
  const pageTitle =
    viewScope === "canceled" ? t("deliveryItems.canceledTitle") : t("deliveryItems.activeTitle");
  ...
  useEffect(() => {
    ...
    async function loadDeliveryItems() {
      ...
      try {
        ...
        if (!isCancelled) {
          setDeliveryItems(
            applyViewScopeToItems(payload.delivery_items ?? [], viewScope),
          );
  ...
  async function handleStatusUpdate(deliveryItemId, nextStatus) {
    const currentItem = deliveryItems.find(
      (item) => item.id === deliveryItemId,
    );
    ...
    if (
      ...
      setDeliveredDialogState({
        ...
        transferredAmount: String(
          currentItem.transferred_to_shop_amount ?? "0",
        ),
        collectedAmount: String(
          currentItem.collected_amount ?? currentItem.price ?? "0",
        ),
      });
    ...
    try {
      const payload = await apiRequest(
        `/api/delivery-items/${deliveryItemId}/status`,
        {
          method: "PATCH",
          token: auth?.token,
          body: JSON.stringify({
  ...
  async function handleDeliveredDialogSubmit(event) {
    ...
    try {
      const payload = await apiRequest(
        `/api/delivery-items/${deliveredDialogState.deliveryItemId}/status`,
        {
          method: "PATCH",
          token: auth?.token,
          body: JSON.stringify({
            delivery_status: "delivered",
            transferred_to_shop_amount: Number(
              deliveredDialogState.transferredAmount || 0,
            ),
      ...
      setDeliveryItems((current) =>
        applyViewScopeToItems(
          current.map((item) =>
            item.id === deliveredDialogState.deliveryItemId
              ? payload.delivery_item
              : item,
          ),
          viewScope,
        ),
  ...
  async function handleCourierUpdate(deliveryItemId, nextCourierId) {
    ...
    try {
      const payload = await apiRequest(
        `/api/delivery-items/${deliveryItemId}/courier`,
        {
          method: "PATCH",
          token: auth?.token,
          body: JSON.stringify({
  ...
  async function handleCourierCommentUpdate(deliveryItemId, nextComment) {
    ...
    try {
      const payload = await apiRequest(
        `/api/delivery-items/${deliveryItemId}/courier-comment`,
        {
          method: "PATCH",
          token: auth?.token,
          body: JSON.stringify({
  ...
  async function handleProductUpdate(deliveryItemId, nextProduct) {
    ...
    try {
      const payload = await apiRequest(
        `/api/delivery-items/${deliveryItemId}/product`,
        {
          method: "PATCH",
          token: auth?.token,
          body: JSON.stringify({
  ...
  async function handlePriceUpdate(deliveryItemId, nextPrice) {
    ...
    try {
      const payload = await apiRequest(
        `/api/delivery-items/${deliveryItemId}/price`,
        {
          method: "PATCH",
          token: auth?.token,
          body: JSON.stringify({
  ...
  async function handleDistrictUpdate(deliveryItemId, nextDistrict) {
    ...
    try {
      ...
      const payload = await apiRequest(
        `/api/delivery-items/${deliveryItemId}/location`,
        {
          method: "PATCH",
          token: auth?.token,
          body: JSON.stringify({
  ...
  async function handleAdditionalStatusUpdate(deliveryItemId, nextStatus) {
    ...
    try {
      const payload = await apiRequest(
        `/api/delivery-items/${deliveryItemId}/additional-status`,
        {
          method: "PATCH",
          token: auth?.token,
          body: JSON.stringify({
  ...
  async function handleWarehouseStateUpdate(deliveryItemId, nextState) {
    ...
    try {
      const payload = await apiRequest(
        `/api/delivery-items/${deliveryItemId}/warehouse-state`,
        {
          method: "PATCH",
          token: auth?.token,
          body: JSON.stringify({
  ...
  function handlePerPageChange(nextPerPage) {
    if (
      !PER_PAGE_OPTIONS.includes(nextPerPage) ||
  ...
  function isTariffPerKgItem(item) {
    if (isAdmin) {
      return Boolean(
        partnerIdMap[String(item.partner_id ?? "")]?.tariff_per_kg,
      );
    }
  ...
  async function downloadAdminImportTemplate() {
    ...
    if (!availablePartners.length) {
      try {
        const partnersPayload = await apiRequest("/api/partners", {
          token: auth?.token,
        });
    ...
    if (!availableCities.length) {
      try {
        const citiesPayload = await apiRequest("/api/cities", {
          token: auth?.token,
        });
    ...
    if (partnerOptionsSheet.rowCount > 0) {
      for (let rowNumber = 2; rowNumber <= 500; rowNumber += 1) {
        worksheet.getCell(`A${rowNumber}`).dataValidation = {
          ...
          allowBlank: false,
          formulae: [
            `'Partner Options'!$A$1:$A$${partnerOptionsSheet.rowCount}`,
          ],
  ...
  function validateImportedItems(items) {
    ...
    items.forEach((item, index) => {
      ...
      if (isAdmin) {
        ...
        if (!partnerId) {
          ...
          nextImportErrors.push(
            `Row ${rowNumber}: selected partner was not found.`,
          );
        }
      }
      ...
      if (!String(item.product ?? "").trim()) {
        ...
        isTariffPerKgItem(item) &&
        Number.isNaN(Number(item.product))
      ) {
        nextImportErrors.push(
          `Row ${rowNumber}: product must be a decimal value for this partner.`,
        );
      }
      ...
      if (!String(item.delivery_date ?? "").trim()) {
        nextImportErrors.push(
          `Row ${rowNumber}: delivery_date must be a valid date.`,
        );
      }
    });
    ...
    if (items.length > 200) {
      nextImportErrors.push(
        "The import file can contain at most 200 rows per upload.",
      );
    }
    ...
    try {
      ...
      const nextImportedItems = rawRows
        ...
        .map((row, index) => {
          ...
          if (isAdmin) {
            const partnerEmail = String(row.partner_email ?? "")
              .trim()
              .toLowerCase();
            const matchedPartner = partnerEmailMap[partnerEmail];

            if (!partnerEmail) {
              partnerImportErrors.push(
                `Row ${rowNumber}: partner_email is required.`,
              );
              return normalizedItem;
            } else if (!matchedPartner) {
              partnerImportErrors.push(
                `Row ${rowNumber}: partner_email "${row.partner_email}" was not found.`,
              );
              return normalizedItem;
            }
  ...
  const activeColumns = visibleColumnDefinitions.filter(
    (column) => visibleColumns[column.key],
  );
  const visibleColumnCount = visibleColumnDefinitions.filter(
    (column) => visibleColumns[column.key],
  ).length;
    ...
    <section className="delivery-items-page">
        ...
        headerActions={
          <>
            <button
              ...
              onClick={() =>
                handleMultiSelectSet(
                  "status",
                  visibleStatusOptions.map(([status]) => status),
                )
              }
            >
              {t("deliveryItems.selectAll")}
            </button>
            <button
              type="button"
              className="delivery-items-table__multi-filter-action"
      ...
      {deliveredDialogState ? (
        <DeliveredStatusDialog
          ...
          onTransferredAmountChange={(value) =>
            setDeliveredDialogState((current) =>
